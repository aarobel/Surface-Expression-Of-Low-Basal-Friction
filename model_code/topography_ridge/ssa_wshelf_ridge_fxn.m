function [params] = ssa_wshelf_ridge_fxn(L, frxn_ramp, bmbi, shelf_only, ridge)
%% Bed parameters
...
% bed ridge 
params.rx = ridge.rx;       % ridge slope
params.rx0 = ridge.rx0;     % ridge position upstream of GL 
...
params.L = L;               % set prescribed bump upstream distance where basal melt reaches (meters)
...
hf = (-bed_ridge(xg.*params.xscale,params)/params.hscale)/(1-params.lambda);    % floatation thickness
h = [1 - (1-hf).*params.sigma(1:params.Nx) ; hf.*(linspace(1,0.5,params.N2)')]; % ice thickness
u = 0.3*(params.sigma_elem.^(1/3)) + 1e-3;  % ice velocity
b = -bed_ridge(xg.*params.sigma.*params.xscale,params)/params.hscale;   % bed elevation
...
[huxg_init,F,exitflag,output,JAC] = fsolve(flf,huxg0,options);  % h, u, xg 
...
u_bl = u_schoof(-bed_ridge(xg.*params.xscale,params)/(1-params.lambda),params); % BL approx for GL flux
...
b = -bed_ridge(xg.*sigma.*params.xscale,params)/params.hscale;
Fh = zeros(Ntot,1);
Fu = zeros(Ntot,1);
...
if params.rx0 ~= 0
    if numel(x) == 1
        xg_here = x.*params.sigma(Nx); % this should be xscaled
    else
        xg_here = x(Nx); 
    end
    rx0 = xg_here - params.rx0; % position where slope change occurs
    all_x = x.*params.sigma;
    b_up = params.b0 + params.bx*all_x(all_x < rx0); % slope upstream of rx0
    b = b_up;
    x_dif = abs(all_x(Nx) - all_x(Nx-1));
    for e = length(b_up)+1:numel(all_x)
        b(e) = b(e-1) + params.rx*x_dif;
    end
    ...
    b = params.b0 + params.bx.*x; % standard slope
end
...
% calculate a function for a basal sliding parameter that linearly increases upstream of the GL until some prescribed bump distance
