...
%% Run L = 0.25 km simulation
L = 250;           % Friction ramp length = 1,000 m
frxn_ramp = true;   % Apply friction ramp
bmbi = 0;           % No basal melt
shelf_only = true;  % Basal melt (if >0) applies on the ice shelf only
[frxn025] = ssa_wshelf_fine_fxn(L, frxn_ramp, bmbi, shelf_only);   % Obtain frxn1 output

%% Run L = 0.5 km simulation
L = 500;           % Friction ramp length = 1,000 m
frxn_ramp = true;   % Apply friction ramp
bmbi = 0;           % No basal melt
shelf_only = true;  % Basal melt (if >0) applies on the ice shelf only
[frxn05] = ssa_wshelf_fine_fxn(L, frxn_ramp, bmbi, shelf_only);   % Obtain frxn1 output
...
save(['frxn025.mat'], 'frxn025', '-mat');
save(['frxn05.mat'], 'frxn05', '-mat');
save(['frxn1.mat'], 'frxn1', '-mat');
save(['frxn5.mat'], 'frxn5', '-mat');
save(['frxn10.mat'], 'frxn10', '-mat');
