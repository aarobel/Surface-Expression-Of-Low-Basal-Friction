...
%% >>> Run the m_i = 5 m/yr simulations <<<

% %% Run the Control simulation (m_i = 1 m/yr)
L = 0;              % Basal melt ramp length = 0
frxn_ramp = false;  % No friction ramp
bmbi = -3;       % 1 m/yr basal melt (ONLY on the ice shelf)
shelf_only = true;  % Basal melt (if >0) applies on the ice shelf only
[ctl_5] = ssa_wshelf_fine_fxn(L, frxn_ramp, bmbi, shelf_only);  % Obtain ctl_1 output

%% Run the L = 1 km simulation (m_i = 1 m/yr)
L = 1000;           % Basal melt ramp length = 1,000 m
frxn_ramp = false;  % No friction ramp
bmbi = -3;       % 1 m/yr basal melt
shelf_only = false; % Basal melt (if >0) applies upstream of ice shelf/GL
[bmb1_5] = ssa_wshelf_fine_fxn(L, frxn_ramp, bmbi, shelf_only); % Obtain bmb1_1 output
...
bmbi = -3;       % 1 m/yr basal melt
shelf_only = false; % Basal melt (if >0) applies upstream of ice shelf/GL
[bmb5_5] = ssa_wshelf_fine_fxn(L, frxn_ramp, bmbi, shelf_only); % Obtain bmb5_1 output
...
bmbi = -3;       % 1 m/yr basal melt
shelf_only = false; % Basal melt (if >0) applies upstream of ice shelf/GL
[bmb10_5] = ssa_wshelf_fine_fxn(L, frxn_ramp, bmbi, shelf_only);    % Obtain bmb10_1 output
